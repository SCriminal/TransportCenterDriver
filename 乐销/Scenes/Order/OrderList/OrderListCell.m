...
- (void)resetCellWithModel:(ModelOrderList *)model{
    ...
    [self.labelPortType fitTitle:[NSString stringWithFormat:@"%@ %@",model.orderType== ENUM_ORDER_TYPE_INPUT?@"进口运单":@"出口运单",model.total == 2?@"双背":@"单背"] variable:self.viewBG.width - self.ivPortType.right - W(40)];
    ...
    [self.labelConnect fitTitle:[NSString stringWithFormat:@"%@：%@",model.orderType == ENUM_ORDER_TYPE_INPUT?@"卸货时间":@"装货时间",[GlobalMethod exchangeTimeWithStamp:model.placeTime andFormatter:TIME_SEC_SHOW]] variable:self.ivUser.left - self.ivUser.right - W(40)];
    ...
    [self.labelOperateTime fitTitle:[NSString stringWithFormat:@"联系人：%@ %@",UnPackStr(model.placeContact),UnPackStr(model.placePhone)] variable:self.ivUser.left - self.ivUser.right - W(40)];
