...
- (UILabel *)labelBillNo{
    if (_labelBillNo == nil) {
        ...
        _labelBillNo.textColor = COLOR_666;
        _labelBillNo.font =  [UIFont systemFontOfSize:F(14) weight:UIFontWeightRegular];
...
- (void)resetCellWithModel:(ModelAccountList *)model{
    ...
    [self.labelBillNo fitTitle:[NSString stringWithFormat:@"提单编号：%@",UnPackStr(model.billNumber)] variable:self.labelPrice.left - W(30)];
    self.labelBillNo.leftTop = XY(self.labelTitle.left,self.labelTitle.bottom+W(15));
    ...
    self.labelTime.leftTop = XY(self.labelTitle.left,self.labelBillNo.bottom+W(15));
...
-(UIButton *)btnManage{
    if (_btnManage == nil) {
        ...
        [_btnManage setTitle:@"管理" forState:(UIControlStateNormal)];
...
@implementation AccountBottomEditView
#pragma mark 懒加载
- (UILabel *)labelSum{
    if (_labelSum == nil) {
        _labelSum = [UILabel new];
