...
-(void)addDataPoints {
    ...
    for (int i = 0; i<arr.count; i++) {
        ...
        detailLabel.hidden = ![self isToday:i-1];
...
-(void)addBottomViews {
    ...
    for (int i = 0;i< _dataArrOfX.count ;i++ ) {
        ...
        UILabel *leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(5+_xMargin/2+i*_xMargin-_xMargin*0.7, (_countY - 1)*_yMargin + 5, _xMargin, 20)];
        leftLabel.font = [UIFont systemFontOfSize:F(10)];
...
#pragma mark 判断是否是今天
- (BOOL)isToday:(NSInteger)index{
    if (index<self.dataArrOfX.count) {
        NSString * strDate = self.dataArrOfX[index];
        if ([strDate isEqualToString:[GlobalMethod exchangeDate:[NSDate date] formatter:@"MM-dd"]]) {
            return true;
        }
    }
    return false;
