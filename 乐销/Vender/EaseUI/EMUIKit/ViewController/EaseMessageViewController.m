...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,UIGestureRecognizerDelegate>
{
    ...
    if ([self.messsagesSource count] && [EMClient sharedClient].options.sortMessageByServerTime) {
        ...
        if ([last isKindOfClass:[EMMessage class]]) {
            ...
            if (index != NSNotFound) {
                ...
                //sld_test  send Message success
                NSArray *conversations = [[EMClient sharedClient].chatManager getAllConversations];
                for (EMConversation * converstion in conversations) {
                    EMMessage * message = converstion.latestMessage;
                    NSLog(@"%@   %d",converstion.ext,message.body.type);
                }
                //格式化消息
                self.messageTimeIntervalTag = -1;
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    //sld_change
    [self shouldSendMessage:message];
...
//sld_change
- (void)shouldSendMessage:(EMMessage *)message{
...
- (NSArray*)_searchAtTargets:(NSString*)text
{
    ...
    if (text.length > 1) {
        ...
        NSArray *splits = [text componentsValidSeparatedByString:@"@"];
        if ([splits count]) {
